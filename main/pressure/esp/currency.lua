...
function Module.CreateTab(Window)
    ...
    local function createVisuals(model)
        ...
        -- Find a part inside the model to attach the billboard to
        local adorneePart = model:FindFirstChildWhichIsA("BasePart")
        if not adorneePart then return end -- Can't create ESP without a part
        ...
        billboard.Adornee = adorneePart -- Attach billboard to the part inside the model
    ...
    -- UPDATED: Function to find currency items
